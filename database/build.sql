...
INSERT INTO `vox_role_admin` (`name`, `remark`, `status`, `create_time`, `update_time`) VALUES
...
CREATE TABLE `vox_user_admin` (
  ...
  `update_time` INT DEFAULT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`),
  FOREIGN KEY (role_id) REFERENCES vox_role_admin(id)
) ENGINE=MyISAM DEFAULT CHARSET=utf8;

INSERT INTO `vox_user_admin` (`username`, `pass`, `role_id`, `remark`, `status`, `login_time`, `create_time`, `update_time`) VALUES
...
INSERT INTO `vox_media_type` (`name`, `status`, `create_time`, `update_time`) VALUES
...
INSERT INTO `vox_media` (`type_id`, `name`, `url`, `status`, `create_time`, `update_time`) VALUES
(1, '微博', 'http://weibo.cn', 1, 1508811608, NULL);

# Dump of table vox_operation_log
# ------------------------------------------------------------

DROP TABLE IF EXISTS `vox_operation_log`;

CREATE TABLE `vox_operation_log` (
  `id` INT unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
...
CREATE TABLE `vox_task` (
  ...
  `match_accuracy` int DEFAULT 3 COMMENT '1->一级强度, 2->二级强度, 3->三级强度',
  `match_type` TINYINT DEFAULT NULL COMMENT  '匹配类型：1->按内容匹配； 2->按主题配置',
  `necessary_keywords` VARCHAR(1000) DEFAULT '' COMMENT  '必要关键词',
  `unnecessary_keywords` VARCHAR(1000) DEFAULT '' COMMENT  '非必要关键词',
  `begin_time` INT DEFAULT NULL COMMENT '开始时间',
...
CREATE TABLE `vox_data` (
  ...
  `url` VARCHAR(100) DEFAULT NULL COMMENT '网址',
  `publish_time` INT DEFAULT NULL COMMENT '发表时间',
