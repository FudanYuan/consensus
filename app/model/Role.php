...
class Role extends Model{
    ...
    public function getById($role_id){
        ...
        $res = $this->field('id,name,remark')
            ...
            ->find();
        if(!empty($res)){
            $actions = Db::table('vox_role_action_admin')
    ...
    public function addRoleAction($role_id, $action_ids){
        ...
        return Db::table('vox_role_action_admin')->insertAll($data);
    }
    ...
    public function getRoleActions($role_id){
        return Db::table('vox_role_action_admin')->where(['role_id' => $role_id, 'status' => 1])->column('action_id');
    }
    ...
    public function removeRoleActions($role_id, $action_ids){
        return Db::table('vox_role_action_admin')->where(['role_id' => $role_id, 'action_id' => ['in', $action_ids]])->delete();
    }
    ...
    public function getActionsByRoleId($role_id){
        if(!$role_id) return [];
        $res = [];
        $actions = Db::table('vox_role_action_admin')->alias('a')->field('b.id,b.name,b.tag,b.pid,b.pids,level')
            ->where(['a.role_id' => $role_id, 'a.status' => 1])->join('vox_action_admin b', 'a.action_id=b.id', 'LEFT')
            ->column('tag');
        return $actions;
    }
}
