...
class Role extends Model{
     ...
     */
 	...
 	public function getRoleActions($roleid){
 		return Db::table('vox__role_action_admin')->where(['roleid' => $roleid, 'status' => 1])->column('actionid');
 	}
     ...
     */
 	public function removeRoleActions($roleid, $actionids){
 		return Db::table('vox__role_action_admin')->where(['roleid' => $roleid, 'actionid' => ['in', $actionids]])->delete();
 	}
 	/**
 	 * 根据角色获取操作列表
 	 */
 	public function getActionsByRoleId($roleid){
 		if(!$roleid) return [];
 		$res = [];
 		$actions = Db::table('vox__role_action_admin')->alias('a')->field('b.id,b.name,b.tag,b.pid,b.pids,level')
 			->where(['a.roleid' => $roleid, 'a.status' => 1])->join('vox__action_admin b', 'a.actionid=b.id', 'LEFT')
 			->column('tag');
 		return $actions;
 	}
 }
